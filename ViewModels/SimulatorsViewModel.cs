...
{
    public class SimulatorsViewModel : ObservableObject
    {
        // Die Liste, an die sich das XAML bindet
        public ObservableCollection<SimulatorItem> SimulatorsList { get; set; }
        private SimulatorItem _selectedSimulator;
        public SimulatorItem SelectedSimulator
        {
            get { return _selectedSimulator; }
            set
            {
                if (_selectedSimulator != value)
                {
                    _selectedSimulator = value;
                    OnPropertyChanged(nameof(SelectedSimulator));
                }
            }
        }
        ...
        public bool IsAllSelected
        {
            ...
            set
            {
                if (_isAllSelected != value)
                {
                    ...
                    {
                        foreach (var item in SimulatorsList)
                        {
        ...
        public SimulatorsViewModel() // Konstruktor
        {
            // Liste initialisieren (noch leer)
            SimulatorsList = new ObservableCollection<SimulatorItem>();
            ...
            try
            {
                ...
                foreach (string ordner in unterordner)
                {
                    ...
                    foreach (string filePath in gbFiles)
                    {
                        ...
                        SimulatorsList.Add(new SimulatorItem
                        {
                            IsChecked = false,
                            Name = simulatorName,
                            Date = backupDate,
        ...
        private void ExecuteSort(object parameter)
        {
            ...
            ICollectionView dataView = CollectionViewSource.GetDefaultView(SimulatorsList);
