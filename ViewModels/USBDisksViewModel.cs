...
{
    public class USBDisksViewModel : ObservableObject
    {
        // Die Liste, an die sich das XAML bindet
        public ObservableCollection<USBDiskItem> USBDisksList { get; set; }
        private USBDiskItem _selectedUSBDisk;
        public USBDiskItem SelectedUSBDisk
        {
            get { return _selectedUSBDisk; }
            set
            {
                if (_selectedUSBDisk != value)
                {
                    _selectedUSBDisk = value;
                    OnPropertyChanged(nameof(SelectedUSBDisk));
                }
            }
        }
        ...
        public bool IsAllSelected
        {
            ...
            set
            {
                if (_isAllSelected != value)
                {
                    ...
                    if (USBDisksList != null)
                    {
                        foreach (var item in USBDisksList)
                        {
        ...
        public USBDisksViewModel() // Konstruktor
        {
            // Liste initialisieren (noch leer)
            USBDisksList = new ObservableCollection<USBDiskItem>();
            ...
            try
            {
                ...
                foreach (string ordner in unterordner)
                {
                    ...
                    foreach (string filePath in gbFiles)
                    {
                        ...
                        USBDisksList.Add(new USBDiskItem
                        {
                            IsChecked = false,
                            Name = simulatorName,
                            Date = backupDate,
        ...
        private void ExecuteSort(object parameter)
        {
            ...
            ICollectionView dataView = CollectionViewSource.GetDefaultView(USBDisksList);
